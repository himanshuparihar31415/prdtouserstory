...
export default function Step11Page() {
  ...
  return (
    <WizardShell step={11} title="Story Interaction Layer" description="Review and edit each story — modify the user role, action, benefit, and acceptance criteria.">
              ...
              {isEditing ? (
                <div className="space-y-3 ml-12">
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <label className="text-xs font-semibold text-brand-text block mb-1">As a</label>
                      <input value={draft.asA} onChange={e => setDraft(d => ({ ...d, asA: e.target.value }))}
                        className="w-full border border-brand-blue rounded-lg px-3 py-2 text-xs outline-none" />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-brand-text block mb-1">I want</label>
                      <input value={draft.iWant} onChange={e => setDraft(d => ({ ...d, iWant: e.target.value }))}
                        className="w-full border border-brand-blue rounded-lg px-3 py-2 text-xs outline-none" />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-brand-text block mb-1">So that</label>
                      <input value={draft.soThat} onChange={e => setDraft(d => ({ ...d, soThat: e.target.value }))}
                        className="w-full border border-brand-blue rounded-lg px-3 py-2 text-xs outline-none" />
                    </div>
                  </div>
                    ...
                    <div className="space-y-2">
                      {editAcs.map((ac, i) => (
                        <div key={i} className="flex gap-2">
                          <input value={ac} onChange={e => updateAc(i, e.target.value)}
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    <button onClick={cancelEdit} className="btn-secondary flex items-center gap-1.5 text-xs">
                      <X size={12} /> Cancel
                    </button>
                  </div>
                </div>
                ...
                <div className="ml-12 space-y-2">
                  ...
                  <div>
                    <p className="text-xs font-medium text-brand-text mb-1">Acceptance Criteria:</p>
      ...
      <StepFooter step={11} canProceed={stories.length > 0} nextLabel="Finalize Stories" />
    </WizardShell>
  );
}
